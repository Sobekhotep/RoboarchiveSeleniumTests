...
·      Enter the word available in the database - " Óôà ". Make sure results are not empty.
·      Enter the word available in the database - " óåçä ".Make sure results are not empty.
·      Enter the word available in the database - " ãóáåğíèÿ ". Make sure results are not empty.
·      Enter the word available in the database - " Îğåíáóğã ". Make sure results are not empty.
·      Enter the word available in the database - " Ìèíñê ". Make sure results are not empty.
 
 
2.       Testing the search in the ÍÈÀÁ database: tick the ÍÈÀÁ field, perform an "action", then click the "Search" button.
·      Do not enter characters. Make sure results are not empty.
...
·      Write two identical letters of the English alphabet "aa". Make sure there are no results on the page.
·      Enter the word available in the ÖÃÈÀ ĞÁ database and absent in the ÍÈÀÁ database - “Óôà”.  Make sure there are no results on the page.
·      Enter the word available in the ÖÃÈÀ ĞÁ database and absent in the ÍÈÀÁ database - "ãóáåğíèÿ ". Make sure there are no results on the page.
·      Enter the word available in the database - " óåçä ". Make sure results are not empty.
·      Enter the word, which is available in the ÖÃÈÀ ĞÁ database and is absent in the ÍÈÀÁ database - “Îğåíáóğã”. Make sure there are no results on the page.
·      Enter the word available in the ÍÈÀÁ database and absent in the ÖÃÈÀ ĞÁ database - “Ìèíñê”. Make sure results are not empty.
 
3. Testing the search in the ÖÃÈÀ ĞÁ database: tick the field of the ÖÃÈÀ ĞÁ, perform the "action", then click the "Search" button.
...
·      Enter the word available in the database - "óåçä". Make sure results are not empty.
·      Enter the word available in the ÖÃÈÀ ĞÁ database and absent in the ÍÈÀÁ database – «Îğåíáóğã». Make sure results are not empty.
·      Enter the word available in the ÍÈÀÁ database and absent in the ÖÃÈÀ ĞÁ database - «Âèëåéêà». Make sure there are no results on the page.
...
Mark the ÍÈÀÁ field with a tick, click the "Enter" button. The URL value is: http://roboarchive.org/search?docdescription=false&doctitle=false&emptysearch=true&fromdate=&gasrb=false&niab=true&page=1&search=&todate=
 
Mark the field of the ÖÃÈÀ ĞÁ by ticking the box, press the "Enter" button. The URL value is: http://roboarchive.org/search?docdescription=false&doctitle=false&emptysearch=true&fromdate=&gasrb=true&niab=false&page=1&search=&todate=
 
Mark the " Çàãîëîâîê äîêóìåíòà " field by ticking, then press "Enter" button. The URL value is: http://roboarchive.org/search?docdescription=false&doctitle=true&emptysearch=true&fromdate=&gasrb=false&niab=false&page=1&search=&todate=
 
 
Mark the " Îïèñàíèå äîêóìåíòà " field, click "Enter". The URL value is: http://roboarchive.org/search?docdescription=true&doctitle=false&emptysearch=true&fromdate=&gasrb=false&niab=false&page=1&search=&todate=
 
Enter  the  value  "1800"  in  the  "Îò"  field and the value "1900" in the "Äî" field of the time range, press the button "Search" (Enter). The URL value is:
 ...
 Enter the value of "Óôà" in the search field, click "Search" (Enter). The URL value is: http://roboarchive.org/search?docdescription=false&doctitle=false&emptysearch=true&fromdate=&gasrb=false&niab=false&page=1&search=%D0%A3%D1%84%D0%B0&todate=
 
Enter the value of "Óôà" in the search field, tick the field ÖÃÈÀ ĞÁ, enter the value "1800" in the field "Îò" and the value "1900" in the "Äî" field of the time range, click "Enter". Select the search page 5. The URL value is: 
